...
public class SurveyFormEmployeeStoreCalendarTargetedMobileQuery() : IRequest<HistoryStoreSurveys>
{
    public int EmployeeId { get; set; }
    public int? StoreId { get; set; }
    public DateTime Date { get; set; }
    public int EmployeeStoreCalendarId { get; set; }
    public List<int> ExistingFormIds { get; set; }

}

public class SurveyFormEmployeeStoreCalendarTargetedMobileHandler(IAppDbContext context, IMapper mapper) : IRequestHandler<SurveyFormEmployeeStoreCalendarTargetedMobileQuery, HistoryStoreSurveys>
{
    private readonly IAppDbContext _context = context;
    private readonly IMapper _mapper = mapper;
    public async Task<HistoryStoreSurveys> Handle(SurveyFormEmployeeStoreCalendarTargetedMobileQuery query, CancellationToken cancellationToken)
    {
        ...
        var targetedSurveyForms = await _context.SurveyFormTargets
                                   .Where(e => e.SurveyForm.SurveyFormTypeId == (int)SurveyFormTypeId.ContactReport
                                        && e.SurveyForm.StartDate.Value.Date <= query.Date.Date
                                        && (e.SurveyForm.EndDate == null || e.SurveyForm.EndDate.Value.Date >= query.Date.Date)
        ...
        var completedSurveys = await _context.SurveyFormSubmissions.Where(e => e.IsComplete
                                                                         ...
                                                                         && e.SurveyForm.SurveyFormType.SurveyFormTypeId == (int)SurveyFormTypeId.ContactReport)
                                                                   .Select(e => e.SurveyFormId)
                                                                   .ToListAsync(cancellationToken);
        ...
        if (query.ExistingFormIds.Any())
        {
            surveyFormIds.AddRange(query.ExistingFormIds);
        }

        surveyFormIds = surveyFormIds.Distinct().ToList();

        var entites = await queryable.Where(e => e.Disabled == false && e.Deleted == false
                                        && e.StartDate.Value.Date <= query.Date.Date
                                        && (e.EndDate == null || e.EndDate.Value.Date >= query.Date.Date)
                                                 && e.SurveyFormQuestionGroups.Any(e => e.SurveyFormQuestions.Count > 0)
                                                 && e.SurveyFormType.SurveyFormTypeId == (int)SurveyFormTypeId.ContactReport
        ...
        var surveyFormSubmissionIds = await _context.EmployeeStoreCalendarSurveyFormSubmissions
            .Where(e => e.EmployeeStoreCalendarId == query.EmployeeStoreCalendarId)
            .Select(e => e.SurveyFormSubmissionId).ToListAsync(cancellationToken);

        //History
        var submissions = await _context.SurveyFormSubmissions
                                                  .Include(e => e.SurveyFormAnswers)
                                                  .Include(e => e.SurveyForm)
                                                  .Where(e => surveyFormSubmissionIds.Contains(e.SurveyFormSubmissionId))
