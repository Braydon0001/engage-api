...
//using StoreConceptAttributeMobile = Engage.Application.Services.Mobile.Database.Models.StoreListDto;
...
public class GetStoreBySearchAndEmployeeRegionQueryHandler : BaseQueryHandler, IRequestHandler<GetStoreBySearchAndEmployeeRegionQuery, List<StoreListDto>>
{
    ...
    public async Task<List<StoreListDto>> Handle(GetStoreBySearchAndEmployeeRegionQuery request, CancellationToken cancellationToken)
    {
        ...
        var employeeDepartmentIds = await _context.EmployeeDepartments.Where(e => e.EmployeeId == request.EmployeeId)
                                                                      .Select(e => e.EngageDepartmentId).Distinct()
        ...
        var stores = await _context.Stores.Join(_context.Locations,
            ...
            (store, location) => new { store, location })
                ...
                .Select(x => new StoreListDto
                {
                    ...
                    Long = x.location.Lat.HasValue ? CheckCoOrdinate(x.location.Long.Value, false).ToFloat() : null,
                    Distance = hasGps && x.location != null && x.location.Lat.HasValue && x.location.Long.HasValue ? (
                                GeoCalculator.GetDistance(
                                gps.Latitude,
                                gps.Longitude,
                                CheckCoOrdinate(x.location.Lat.Value, true),
                                CheckCoOrdinate(x.location.Long.Value, false), 2,
        ...
        {
            var attributesWithScores = await GetStoreAttributes(employeeStoreConceptIds, store.Id, cancellationToken);
            store.Concepts = attributesWithScores.Concepts;
            store.StorePerformancePercent = attributesWithScores.RobotScoresAverage;
        }

        if (!employeeRegions.Contains(7)) // 7 is for all regions.
        {
            stores = stores.Where(x => employeeRegions.Contains(x.EngageRegionId)).ToList();
        }
