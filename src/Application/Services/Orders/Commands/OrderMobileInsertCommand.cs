...
public class OrderMobileInsertCommand : IRequest<OperationStatus>
{
...
public class OrderMobileInsertHandler : BaseCreateCommandHandler, IRequestHandler<OrderMobileInsertCommand, OperationStatus>
{
    private readonly IUserService _user;
    private readonly OrderDefaultsOptions _options;

    public OrderMobileInsertHandler(IAppDbContext context, IMapper mapper, IMediator mediator, IUserService user, IOptions<OrderDefaultsOptions> options)
    ...
    public async Task<OperationStatus> Handle(OrderMobileInsertCommand request, CancellationToken cancellationToken)
    {
        ...
        var userGroups = await _context.UserUserGroups.IgnoreQueryFilters()
                                                      .Include(e => e.UserGroup)
                                                      .Where(e => e.UserId == user.UserId && !e.Deleted && !e.Disabled)
                                                      .Select(e => e.UserGroup.Name)
        ...
        foreach (var dtoProduct in request.Products)
        {
            ...
            else
            {
                ...
                orderSkus.Add(new OrderSku
                {
                    ...
                    OrderSkuStatusId = 1,
                    OrderSkuTypeId = 1,
                    Note = dtoProduct.Note,
                    // default to case if not specified
                    OrderQuantityTypeId = dtoProduct.OrderQuantityTypeId == 0 ? 3 : dtoProduct.OrderQuantityTypeId,
