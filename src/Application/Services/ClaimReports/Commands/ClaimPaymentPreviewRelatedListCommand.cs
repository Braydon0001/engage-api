...
public class ClaimPaymentPreviewRelatedListCommand : GetQuery, IRequest<ReportListVM<ReportPaymentPreviewDto>>
{
    //Required
...
public class ClaimPaymentPreviewRelatedListCommandHandler : BaseQueryHandler, IRequestHandler<ClaimPaymentPreviewRelatedListCommand, ReportListVM<ReportPaymentPreviewDto>>
{
    private readonly ClaimSettings _claimSettings;
    public ClaimPaymentPreviewRelatedListCommandHandler(IAppDbContext context, IMapper mapper, IOptions<ClaimSettings> claimSettings) : base(context, mapper)
    {
        _claimSettings = claimSettings.Value;
    }

    public async Task<ReportListVM<ReportPaymentPreviewDto>> Handle(ClaimPaymentPreviewRelatedListCommand command, CancellationToken cancellationToken)
    {
        ...
        {
            query = query.Where(e => e.Store.StoreOwners
                                                    .Where(o => o.IsPrimaryOwner == false && o.StartDate.Date <= e.ClaimDate.Date &&
                                                               (o.EndDate == null || o.EndDate.Value.Date >= e.ClaimDate.Date))
                                                    .Any());
        }
        else
        {
            query = query.Where(e => e.Store.StoreOwners
                                                    .Where(o => o.IsPrimaryOwner == true && o.StartDate.Date > e.ClaimDate.Date &&
                                                               (o.EndDate == null || o.EndDate.Value.Date < e.ClaimDate.Date))
                                                    .Any() || !e.Store.StoreOwners.Any());
        }
