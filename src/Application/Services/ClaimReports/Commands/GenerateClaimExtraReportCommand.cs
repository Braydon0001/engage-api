...
public class GenerateClaimExtraReportCommand : GetQuery, IRequest<ReportListVM<ClaimReportDto>>
{
    //Required
...
public class GenerateClaimExtraReportCommandHandler : BaseQueryHandler, IRequestHandler<GenerateClaimExtraReportCommand, ReportListVM<ClaimReportDto>>
{
    public GenerateClaimExtraReportCommandHandler(IAppDbContext context, IMapper mapper) : base(context, mapper)
    {
    }

    public async Task<ReportListVM<ClaimReportDto>> Handle(GenerateClaimExtraReportCommand command, CancellationToken cancellationToken)
    {
        ...
        var query = _context.ClaimSkus
                                      .Where(e => command.ClaimClassificationIds.Contains(e.Claim.ClaimClassificationId) &&
                                                  ...
                                                  && e.Claim.ClaimStatusId == command.ClaimStatusId);

        if (command.ClaimSupplierStatusId.HasValue)
        {
            query = query.Where(e => e.Claim.ClaimSupplierStatusId == command.ClaimSupplierStatusId);
        }

        if (command.IsPayStore)
        {
            query = query.Where(e => e.Claim.IsPayStore == true);
        }

        if (command.IsClaimFromSupplier)
        {
            query = query.Where(e => e.Claim.IsClaimFromSupplier == true);
        }
        ...
        if (command.ToClaimPeriodId.HasValue)
        {
            ...
            if (command.ClaimStatusId == (int)ClaimStatusId.Approved || command.ClaimStatusId == (int)ClaimStatusId.Paid)
            {
            ...
            {
                query = query.Where(e => (e.Claim.Created.Date >= claimPeriod.StartDate.Date && e.Claim.Created.Date <= toClaimPeriod.EndDate.Date));
            }
        }
        else
        {
            ...
            {
                query = query.Where(e => (e.Claim.Created.Date >= claimPeriod.StartDate.Date && e.Claim.Created.Date <= claimPeriod.EndDate.Date));
            }
        }
