...
public class SurveyFormSubmissionSummaryQuery : IRequest<SurveyFormSubmissionSummaryVm>
{
    public int Id { get; set; }
    public bool FullSurvey { get; set; } = false;
}
public record SurveyFormSubmissionSummaryHandler(IAppDbContext Context, IMapper Mapper) : IRequestHandler<SurveyFormSubmissionSummaryQuery, SurveyFormSubmissionSummaryVm>
{
    public async Task<SurveyFormSubmissionSummaryVm> Handle(SurveyFormSubmissionSummaryQuery query, CancellationToken cancellationToken)
    {
        ...
        var mappedEntity = Mapper.Map<SurveyFormSubmissionSummaryVm>(entity);
        ...
        if (query.FullSurvey)
        {
            ...
            foreach (var question in unansweredQuestions)
            {
                ...
                mappedEntity.Answers.Add(Mapper.Map<SurveyFormSubmissionSummaryQuestionVm>(question));
            }
        }
