using Engage.Application.Services.Mobile.ProjectFiles.Commands;
...
public partial class ProjectFileController : BaseMobileController
{
    [HttpGet]
    ...
    [HttpDelete("file/{projectId}")]
    public async Task<ActionResult> FileDelete([FromRoute] int projectId, [FromQuery] string fileName, [FromQuery] string filetype, CancellationToken cancellationToken)
    {
        var result = await Mediator.Send(new ProjectFileFileDeleteMobileCommand
        {
            Id = projectId,
        ...
        return result == false ? NotFound() : Ok(new OperationStatus(projectId));
    }
