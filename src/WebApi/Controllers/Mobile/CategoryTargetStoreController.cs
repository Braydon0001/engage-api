...
public partial class CategoryTargetStoreController : BaseMobileController
{
    [HttpGet]
    ...
    public async Task<ActionResult<ListResult<CategoryTargetDto>>> GetByStore([FromRoute] CategoryTargetStoreByStoreQuery query, CancellationToken cancellationToken)
    {
        var entities = await Mediator.Send(query, cancellationToken);
