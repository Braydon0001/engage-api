...
public class StoreConceptFormController : BaseMobileController
{
    [HttpGet("{employeeId}/{storeId}")]
    public async Task<ActionResult<SurveyFormMobileDto>> ConceptSurveys([FromRoute] int employeeId, [FromRoute] int storeId, CancellationToken cancellationToken)
    {
        ...
        var entity = await Mediator.Send(new SurveyFormStoreConceptMobileQuery() { EmployeeId = employeeId, StoreId = storeId }, cancellationToken);
