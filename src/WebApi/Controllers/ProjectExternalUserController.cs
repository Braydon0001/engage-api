...
public partial class ProjectExternalUserController : BaseController
{
    [HttpGet]
    ...
    [HttpPost("validate")]
    public async Task<IActionResult> Validate(ProjectExternalUserValidateCommand command, CancellationToken cancellationToken)
    {
        var valid = await Mediator.Send(command, cancellationToken);
